...
option_list=list(
  ...
  make_option("--name",default="GEM",action="store",type='character',help="sample"))

opt=parse_args(OptionParser(option_list=option_list))

print(paste0("Sample to process: ", opt$sample))

data <- Read10X(data.dir = opt$sample)
data <-  CreateSeuratObject(counts=data)

saveRDS(data, paste0(opt$name, "_cellranger_seurat.rds"))