...
Channel .fromFilePairs(params.input_kallisto2).view()                                   // the files emited are collected in pairs into a tuple. 
        ...
        .map {tag, pair ->                                                              // each sample occurs twice. this needs to change to 1.  a tag opperator is used to group the samples
                subtags = (tag =~ /(.*)(?=_S)/);                                        // first you write a pattern to match folowed by \d to specify a wild card.                                    
...
process kallisto_bus{
        ...
        input:
        tuple val (name), file (read) from read_files_kallisto
        ...
        """
         kallisto bus  -i $index \
                       ...
                       -x '10xv2' -t ${task.cpus} \
                       ${read} 2>&1 | tee ${name}_kallisto.log
        """
}
