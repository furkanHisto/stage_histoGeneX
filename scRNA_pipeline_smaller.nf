...
sampleChannel = Channel.fromPath("${params.input_cellranger2}/*.gz")
    .ifEmpty { exit 1, "Samples not found: ${params.input_cellranger2}" }
    .map { Path path -> 
        path.toFile()
...
//channel for fastqc
samples_fastqc = Channel.fromPath("${params.input_kallisto2}")




//channel to read the sample files
Channel .fromFilePairs(params.input_kallisto2)                                           // the files emited are collected in pairs into a tuple. 
...
process gene_map{
        ...
        publishDir "${params.outdir}/kallisto/gene_map2", mode : 'copy'
